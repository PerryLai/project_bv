...
def depth_to_local_point_cloud(image, color=None, max_depth=0.5, high=2):
    """
    Convert an image containing CARLA encoded depth-map to a 2D array containing
    ...
    p3d_transposed = numpy.transpose(p3d)
    # Original Y and Z are mix arrange
    p3d_transposed[:, 0] = -p3d_transposed[:, 0]
    p3d_transposed[:, [1,2]] = p3d_transposed[:, [2,1]]
    p3d_filter = p3d_transposed
    ...
    if color is not None:
        ...
        return sensor.PointCloud(p3d_filter, color_array=color)
    # [[X1,Y1,Z1],[X2,Y2,Z2], ... [Xn,Yn,Zn]]
    return sensor.PointCloud(p3d_filter)
