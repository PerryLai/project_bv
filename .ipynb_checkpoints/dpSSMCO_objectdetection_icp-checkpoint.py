...
for index in range(100):
    ...
    for i, items in enumerate(dp_result['items']):
        ...
        for item in items:
            ...
            source_pcd.points = o3d.utility.Vector3dVector(vehicle_covpc_list[item].points)
            trans_final = icp(source_pcd, target_pcd)
            t_x = np.array(trans_final)[0, 3]
            t_y = np.array(trans_final)[1, 3]
            x = vehicle_trans_list[item][0, 3]
            y = vehicle_trans_list[item][1, 3]
            error = math.sqrt((t_x-x)**2+(t_y-y)**2)
            if error < threshold:
                print(item, "correct", error)
                source_pcd = o3d.geometry.PointCloud()
                source_pcd.points = o3d.utility.Vector3dVector(vehicle_pc_list[item].points)
