...
for i in range(100):
    ...
    for index, vehicle_info in enumerate(vehicle_info_list):
        ...
        vehicle_seen_area_list.append(area)
    
    area_table, items = knapSack(51200, files_size, vehicle_seen_area_list)
    area_table = [area_table[5120*i] for i in range(1,11)]
    items = [items[5120*i] for i in range(1,11)]
    print("Area", area_table)
    print("Item", items)
    with open(os.path.join(file_path,"late_fusion_dp_icp_result.json"), "w") as outfile:
