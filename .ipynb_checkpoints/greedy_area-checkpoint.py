...
total_area_list = [0 for i in range(10)]
...
for i in range(100):
    ...
    print(rate_list)
    print(vehicle_seen_area_list)
    acc_rate = 0
    area_list = [0]

    for rate in [50*i for i in range(1,11)]:
        area = 0
        while len(rate_list) > 0 and (acc_rate + rate_list[0]) < rate:
            acc_rate += rate_list[0]
            area += vehicle_seen_area_list[0]
            rate_list.pop(0)
            vehicle_seen_area_list.pop(0)
        area_list.append(area_list[-1] + area)
    
    area_list.pop(0)
    for i in range(10):
        total_area_list[i] += area_list[i]

for i in range(10):
    total_area_list[i] /= 100
